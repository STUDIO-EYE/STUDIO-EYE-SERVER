...
class RequestServiceTest {
	...
	void retrieveRequestPageFail() {
		// given
		Pageable pageable = PageRequest.of(-1, 10); // 잘못된 페이지
		when(requestRepository.findAll(pageable)).thenThrow(new IllegalArgumentException("Page index must not be less than zero"));

		// when
		IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () ->
				requestService.retrieveRequestPage(-1, 10)
		);
